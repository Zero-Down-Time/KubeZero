...
//
// Requires plugins: Job DSL, Gitea, Pipeline Utility Steps, Basic Branch Build
// Strategies (generated jobs build regular branches + PRs + tags). GiteaSCMSource carries
...
def call(Map config = [:]) {
    ...
    node('admin-dsl') {
        ...
        stage('Generate jobs') {
            ...
            // paths' ancestors, deduped. Use .add (not <<, which the Groovy sandbox
            // rejects). Plain lexicographic sort (NOT sort{closure} — that native
            // comparator call is CPS-unsafe and throws "expected to call ArrayList.sort
            // but wound up catching CpsClosure2.call") already yields parents before
            // children: an ancestor path is a proper string prefix of its descendants,
            // and a proper prefix always sorts before the longer string.
            def folderPaths = []
            services.each { svcPath ->
                def segs = "${jobFolder}/${svcPath}".tokenize('/')
                (1..<segs.size()).each { n -> folderPaths.add(segs[0..<n].join('/')) }
            }
            folderPaths = folderPaths.unique().sort()
    ...
    configure { project ->
        (project / 'sources' / 'data') << {
            'jenkins.branch.BranchSource' {
                ...
                // A discovered tag is NOT auto-built without a TagBuildStrategy, so a
                // release tag would sit unbuilt.
                buildStrategies {
                    'jenkins.branch.buildstrategies.basic.AnyBranchBuildStrategyImpl' {
                        strategies {
                            'jenkins.branch.buildstrategies.basic.BranchBuildStrategyImpl'()          // regular branches
                            'jenkins.branch.buildstrategies.basic.ChangeRequestBuildStrategyImpl'()   // PRs (origin + fork)
                            'jenkins.branch.buildstrategies.basic.TagBuildStrategyImpl' {             // release tags
                                atLeastMillis('-1')
                                atMostMillis('604800000')   // build tags up to 7d old — avoids a build storm re-indexing ancient tags
                            }
                        }
                    }
                }
